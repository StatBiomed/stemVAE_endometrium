...
def plot_on_each_test_donor_continueTime(golbal_path, file_path, donor_list, original_predict_donors_dic, latent_dim,
    ...
    # Save the figure
    if model_name == "GPLVM":
        save_file_name = "{}/{}/GPLVM_plot_on_all_test_donor_latent_dim{}_time{}{}".format(golbal_path, file_path,
                                                                                           latent_dim,
                                                                                           time_standard_type,
                                                                                           special_str)
    else:
...
def colors_tuple():
    ...
    """
    # 定义要使用的颜色映射名称列表
    cmap_names = ["Set1", "tab10", "Dark2", "Set2", "Set3", "tab20", "Pastel1",
                  "tab20c", "Pastel2", "tab20b", "Accent"]
    # 使用列表推导式生成颜色列表
...
def plot_on_each_test_donor_continueTime_windowTime(golbal_path, file_path, donor_list, original_predict_donors_dic, latent_dim,
                                                    time_standard_type, special_str="", model_name="",
                                                    label_orginalAsKey_transAsValue_dic="", cell_time=None, special_path_str="",
                                                    plot_subtype_str="type", plot_on_each_cell_type=True,
...
def plot_training_loss_for_tags(tb_logger, plot_tag_list, special_str=""):
    """
...
def calculate_real_predict_corrlation_score(real_time, pesudo_time, only_str=True):
    """Input is dic """
    real_time = list(real_time)
    pesudo_time = list(pesudo_time)
    from scipy import stats
    from sklearn.metrics import r2_score
    stats_result = dict()
    try:
        stats_result["spearman"] = stats.spearmanr(real_time, pesudo_time)
        stats_result["pearson"] = stats.pearsonr(real_time, pesudo_time)
        stats_result["kendalltau"] = stats.kendalltau(real_time, pesudo_time)
        stats_result["r2"] = r2_score(real_time, pesudo_time)
    except:
...
def umap_vae_latent_space_forYiming(adata):
    """
    2023-11-02 21:49:18 for yiming data umap
    :param adata:
    :return:
    """
    data = adata.X
    data_label = adata.obs['day'].values
    label_unique = np.unique(data_label)
    label_mapping = {label_unique[i]: i for i in len(label_unique)}
    colors = colors_tuple()
    plt.figure(figsize=(12, 7))
